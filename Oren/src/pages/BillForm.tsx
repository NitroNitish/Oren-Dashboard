...
export default function BillForm() {
  ...
  const saveBill = async () => {
    ...
    const number = id ? billNumber : generateBillNumber();
    ...
    try {
      ...
      toast.success(id ? "Bill updated successfully!" : "Bill saved successfully!");
      navigate("/history");
    } catch (error) {
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize(14);
    doc.text(id ? billNumber : generateBillNumber(), 15, headerY);
    ...
    yPos += 7;
    doc.setFontSize(10);
    doc.text("Paid Amount:", 120, yPos);
    doc.text(formatCurrency(paidAmount), 160, yPos, { align: "left" });
    yPos += 7;
    doc.setTextColor(211, 47, 47);
    doc.text("Balance Due:", 120, yPos);
    doc.text(formatCurrency(grandTotal - paidAmount), 160, yPos, { align: "left" });
    
    // Payment status
    doc.setTextColor(0, 0, 0);
    yPos += 10;
    yPos = checkPageBreak(yPos, 40);
    doc.text(`Payment Status: ${paymentStatus.toUpperCase()}`, 15, yPos);
    ...
    doc.save(`Bill-${id ? billNumber : generateBillNumber()}.pdf`);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <p className="text-lg">Loading bill...</p>
      </div>
    );
  }
      ...
      <main className="container mx-auto px-4 py-8 max-w-5xl">
        <div className="bg-card text-card-foreground shadow-xl rounded-lg p-10 border border-border">
          ...
          <div className="grid grid-cols-3 gap-6 mb-8">
            ...
            <div>
              <Label className="text-base font-semibold">Paid Amount (₹)</Label>
              <Input
                type="number"
                value={paidAmount}
          ...
          <div className="flex gap-4">
            <Button onClick={saveBill} className="flex-1 h-12 text-base">
